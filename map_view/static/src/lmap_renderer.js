...
export class LeafletMapRenderer extends Component {
    static template = 'map_view.MapRenderer'
    static props = {
        model: Object,
        archInfo:Object,
    }
    ...
    createMarker(record) {
        if(record[this.latitude]==0 &&record[this.longitude]==0) {
            return
        }
        ...
        html+=`<h3>${record.name}</h3><br><br/><button id="leafletMapPopupOpenBtn" data-res-id='${record.id}' class='btn btn-primary' >open</button>`
